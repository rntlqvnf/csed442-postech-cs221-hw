...
class BayesNet(object):
    ...
    def sameGraph(self, other):
        ...
        sameVarsDomain = sorted(self.__variableDomainsDict.items()) == sorted(other.__variableDomainsDict.items())
        return sameVars and sameInEdges and sameOutEdges and sameVarsDomain
    ...
    def linearizeVariables(self):
        ...
        if hasEdgesLeftOver:
            raise ValueError("Graph has at least one cycle (not a bayes net) " + str(inEdgesIncremental))
        else:
...
class Factor(object):
    ...
    def __getAssignmentsInOrder(self, assignmentDict):
        ...
        variablesAndAssignments = sorted(variablesAndAssignments, \
                                         key=lambda x : self.__variableOrders[x[0]])
