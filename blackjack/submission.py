...
class BlackjackMDP(util.MDP):
    ...
    def succAndProbReward(self, state, action):
        ...
        if action == 'Quit' or sum(deckCardCounts) == 0:
            ...
            for index, card in enumerate(deckCardCounts):
                if card > 0:
                    prob = float(card)/sum(deckCardCounts)
            ...
            if nextCardIndexIfPeeked is not None:
                newDeck = removeCardFromDeck(nextCardIndexIfPeeked)
                newTotalCardValueInHand = self.cardValues[nextCardIndexIfPeeked] + totalCardValueInHand
                if newTotalCardValueInHand > self.threshold:
                    nextState = (newTotalCardValueInHand, None, None)
                    reward = 0
                elif sum(newDeck) == 0:
                    nextState = (newTotalCardValueInHand, None, None)
                ...
                for index, card in enumerate(deckCardCounts):
                    if card > 0:
                        prob = float(card)/sum(deckCardCounts)
                        newDeck = removeCardFromDeck(index)
                        newTotalCardValueInHand = self.cardValues[index] + totalCardValueInHand
